...
/**
 * Wheel Strategy Calendar Component
 *
 * Displays daily premium collected from covered calls and cash-secured puts.
 * Shows assignments, expirations, and early closes with weekly totals.
 */

import { useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { Zap, TrendingUp, X, Target, Clock } from 'lucide-react';
...
async function fetchWheelCalendar(
  year: number,
  month: number,
  accountId?: string | null
): Promise<WheelCalendarData> {
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch wheel calendar');
  }
...
export function WheelCalendar({ accountId }: WheelCalendarProps) {
  const now = new Date();
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['wheelCalendar', year, month, accountId],
    queryFn: () => fetchWheelCalendar(year, month, accountId),
  });
  ...
  // Check if a date is in the future (no data expected)
  ...
  const renderDayCell = (
    ...
    // BaseCalendar creates placeholder days via `as TDay` — premiumCollected is undefined for those
    ...
    return (
      <button
        ...
        className={cn(
          ...
          // Past inactive days get subtle bg, future days stay transparent
      ...
      >
        ...
        {/* Premium amount - only for days with actual non-zero premium */}
  ...
  return (
    <>
      <BaseCalendar
        ...
        days={data?.days ?? []}
        isLoading={isLoading}
        error={error ? 'Failed to load wheel calendar' : null}
        ...
        subtitle={data ? `${formatSignedCurrency(data.summary.totalPremium)} this month` : undefined}
        helpTooltip={{
          title: 'Wheel Strategy Calendar',
          description:
            'Shows daily premium collected from covered calls and cash-secured puts. Click a day or week to see trade details.',
        }}
      />
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {trade.strategy === 'covered_call' ? 'CC' : 'CSP'}
                    </span>
                  </div>
...
function WeekModal({ week, summary, onClose }: WeekModalProps) {
        ...
        <div className="bg-linear-to-br from-violet-500 to-violet-600 p-4 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">Week Summary</span>
            </div>
            <button
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-white/20 transition-colors"
