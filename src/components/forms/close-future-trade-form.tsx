/**
 * Close Future Trade Form Component
 *
 * Allows users to close an open futures position by entering exit details.
 * Mirrors the stock close flow but uses futures-specific labels.
 */
...
import { Activity, DollarSign, Calendar, FileText, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Textarea } from '@/components/ui/textarea';
import { DatePicker } from '@/components/ui/date-picker';
import { closeFutureTradeSchema, type CloseFutureTradeInput } from '@/lib/validations/close-trade';
...
const formLogger = createChildLogger({ module: 'close-future-trade-form' });
...
interface CloseFutureTradeFormProps {
  position: Position;
  onCancel?: () => void;
}

/**
 * Close an open futures position via API.
 */
async function closeFutureTrade(positionId: string, data: CloseFutureTradeInput) {
...
export function CloseFutureTradeForm({ position, onCancel }: CloseFutureTradeFormProps) {
  ...
  } = useForm<CloseFutureTradeInput>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(closeFutureTradeSchema) as any,
    defaultValues: {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const mutation = useMutation({
    mutationFn: (data: CloseFutureTradeInput) => closeFutureTrade(position.id, data),
    onSuccess: () => {
      formLogger.info({ symbol: position.symbol }, 'Future trade closed successfully');
      queryClient.invalidateQueries({ queryKey: ['positions'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
      router.push('/dashboard');
    },
    onError: (err: Error) => {
      formLogger.error({ error: err }, 'Failed to close future trade');
      setError(err.message);
    },
  });

  const onSubmit = (data: CloseFutureTradeInput) => {
    setError(null);
    formLogger.debug({ symbol: position.symbol }, 'Submitting future trade close');
    ...
    <div className="w-full max-w-lg mx-auto">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                {position.side.charAt(0).toUpperCase() + position.side.slice(1)} Futures Position
              </p>
            </div>
          </div>
          {onCancel && (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800">
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Avg Entry</p>
            <p className="font-semibold text-zinc-900 dark:text-zinc-50">
              ${position.avgPrice.toFixed(2)}
            </p>
          </div>
          <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800">
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Notional</p>
            <p className="font-semibold text-zinc-900 dark:text-zinc-50">
